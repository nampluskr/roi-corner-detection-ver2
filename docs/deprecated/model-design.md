...
code는 `reg`, `seg`, `det`로 정리하고, dense geometric prediction, refinement, rule-based
pipeline은 별도 구현 그룹으로 둔다.

## 1. 문서 목적과 범위

이 설계의 목적은 다음과 같다.

- backbone, decoder와 head를 문자열 설정으로 교체한다.
...
`global_dim`, `spatial_channels`, `spatial_stride`, `stage_channels`, `stage_strides`다. Decoder와 head
factory는 이 metadata로 layer 크기를 결정하고 지원하지 않는 조합을 model 생성 전에 거부한다.
...
| `CustomBackbone` | custom encoder stage를 계산해 native final feature와 stage feature를 반환한다. |
| `FeatureExtractor` | backbone과 adapter를 조립해 `FeatureBundle`을 반환한다. |
| `DenseDecoder` | `spatial`과 `stages`를 dense head가 사용할 해상도와 channel로 복원한다. |
...
`FeatureExtractor`는 `backbone` 문자열로 encoder와 전용 adapter를 선택한다. `CustomBackbone`은
이 안에서 선택할 수 있는 custom encoder module이고 `FeatureExtractor`와 동의어가 아니다. 가중치
경로는 backbone 이름과 분리하며, local file, package cache 또는 명시적 download directory를
사용할 수 있다.
...
`CustomBackbone`은 `backbone="custom"`으로 선택하는 pretrained weight 없는 기본 encoder다. GAN
discriminator, U-Net 또는 autoencoder encoder에서 사용하는 반복 convolution block을 참고하되
특정 task의 decoder와 output layer는 포함하지 않는다.
...
기본 `ConvBlock`은 `3 x 3 Conv2d`, normalization, activation과 선택적 stride-2 downsampling으로
...
Dense decoder는 feature 해상도 복원과 skip fusion을 담당하고 prediction head는 필요한 feature
field 또는 decoded feature를 method별 raw output으로 변환한다. Target 생성, loss와 corner decode는
decoder나 head의 책임이 아니다.

### 4.1. Dense decoder

`DenseDecoder`는 `decoder` 문자열로 선택하고 `FeatureSpec`에 필요한 capability를 선언한다. 초기
decoder와 요구 feature는 다음과 같다.

| decoder | 입력 feature | 역할 |
|---|---|---|
| `none` | `global` 또는 `spatial` | coordinate head가 feature를 직접 사용한다. |
| `plain` | 마지막 `spatial` | skip 없이 dense feature를 upsampling한다. |
| `unet` | `spatial`, `stages` | encoder stage와 additive 또는 concatenation skip을 사용한다. |
| `fpn` | 여러 `stages` | lateral projection과 top-down multi-scale feature를 구성한다. |

Decoder의 기본 upsampling은 interpolation 뒤 convolution을 적용하는 `interpolate_conv`고,
`transposed_conv`는 비교 variant다. `CustomSegModel`은 U-Net으로 고정하지 않으며 decoder와 skip
설정은 독립 실험 속성으로 기록한다.

### 4.2. Coordinate GAP head
...
`head="heatmap"`은 decoded spatial feature에서 네 corner별 heatmap을 출력한다. CNN 또는 Swin의
`spatial`과 `stages`를 dense decoder가 target heatmap 해상도로 upsampling한다. ViT와 DINOv2는
single-scale token grid에 별도 decoder가 필요하므로 초기 compatibility 목록에서 제외한다.

### 4.5. Mask head

`head="mask"`는 dense decoder output을 binary panel mask logits로 projection한다. Rounded OLED의
실제 mask를 감독하고, postprocessor는 원호와 holder 접촉부를 제외한 네 직선 변을 fitting해 가상
corner를 계산한다.
...
| decoder | plain, U-Net additive skip, U-Net concatenation skip, 조건부 FPN |
...
model factory는 문자열 설정을 registry entry로 해석하고, backbone adapter, decoder와 head의
capability를 검증한 뒤 model을 조립한다.

### 7.1. `backbone`, `decoder`와 `head` 문자열 registry
...
허용한 조합 수다. Dense decoder의 추가 compatibility는 다음과 같다.

| decoder | 필요한 capability | 허용 head |
|---|---|---|
| `none` | head별 `global` 또는 `spatial` | `coord_gap`, `coord_spatial` |
| `plain` | `spatial` | `heatmap`, `mask`, `line_map` |
| `unet` | `spatial`, `stages` | `heatmap`, `mask`, `line_map` |
| `fpn` | multi-stage `stages` | `heatmap`, `mask`, 조건부 `line_map` |

지원 조합에는 shape가 다른 경우에도 `FeatureSpec` 기반 projection과 decoder를 factory가 구성한다.
`stages`가 없는 backbone에 U-Net이나 FPN을 요청하거나 plain decoder에 skip connection을 요청하면
model 생성 단계에서 명시적 오류를 반환한다.
...
backbone 비교에서는 `method=reg`, `target=corners`, `decoder=none`, `head=coord_gap`, loss,
postprocess, input size, optimizer와 data split을 고정한다. Pretrained backbone과 from-scratch
`CustomBackbone`의 차이는 별도 metadata와 결과 column으로 기록한다.

### 8.3. Decoder, head 및 postprocess 비교 설정

Head 비교에서는 backbone, decoder, initialization, target과 training condition을 고정한다.
Decoder 비교에서는 backbone, head, target, output resolution과 training condition을 고정한다.
Skip connection의 첫 비교는 plain과 U-Net additive skip이며 U-Net concatenation, FPN과
transposed convolution은 별도 ablation으로 둔다. Postprocess 비교에서는 같은 raw-output
checkpoint를 사용한다. `seg`의 four-side fitting과 다른 mask postprocess는 같은 mask logits 또는
저장된 prediction을 입력으로 사용한다.

## 9. 구현 순서와 검증 기준

구현 순서는 다음과 같다.

1. `ConvBlock`, `DeconvBlock`, `FeatureBundle`, `FeatureSpec`과 capability contract를 정의한다.
2. `CustomBackbone`, adapter와 `decoder=none`, `head=coord_gap`인 `reg` baseline을 구현한다.
3. ResNet-18 adapter를 추가하고 `CustomBackbone`과 같은 head로 forward 및 training을 비교한다.
4. `coord_spatial`, plain decoder, `heatmap`과 `mask` head를 순서대로 추가한다.
5. U-Net additive skip을 추가하고 plain decoder와 비교한다.
6. U-Net concatenation, FPN과 transposed convolution을 독립 ablation으로 추가한다.
7. torchvision CNN, ViT, Swin, DINOv2 adapter를 compatibility table에 따라 추가한다.
8. `seg` whole-model adapter와 rule-based four-side fitting을 연결한다.
9. custom 및 whole-model detection, refinement와 classical pipeline을 별도 그룹으로 추가한다.

component 검증은 다음 조건을 만족해야 한다.

- 모든 registered backbone은 선언한 `FeatureSpec`과 실제 feature shape가 일치한다.
- 모든 허용 backbone-decoder-head 조합은 batch size 1과 2에서 forward가 성공한다.
- 지원하지 않는 조합과 알 수 없는 문자열은 생성 시 명시적 오류를 반환한다.
- Plain, U-Net과 FPN decoder는 선언한 output shape와 skip capability를 만족한다.
