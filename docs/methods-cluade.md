...
이 문서는 평면 사각형 코너 검출 방법론의 공통 계약, 제약조건, 분류 체계를 정의하는 SSOT다.
입력 이미지와 4개 코너 좌표를 받아 표준 코너 4개를 출력하고 공통 메트릭으로 평가한다는
공통 계약을 먼저 확정하고, 그 위에서 제약조건과 방법론 목록을 원점에서 재도출한다. ver1과
ver2의 코드는 "이렇게 구현할 수 있다"는 구현 예시로만 인용하며, 규약의 근거로 삼지 않는다.
